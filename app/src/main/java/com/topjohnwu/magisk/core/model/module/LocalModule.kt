        ...
        set(enable) {
            ...
            if (enable) {
                ...
                if (Const.Version.atLeast_21_2())
                    Shell.jojo("copy_sepolicy_rules").submit()
                else
                    Shell.jojo("mkdir -p $dir", "cp -af $ruleFile $dir").submit()
            } else {
                !disableFile.createNewFile()
                if (Const.Version.atLeast_21_2())
                    Shell.jojo("copy_sepolicy_rules").submit()
                else
                    Shell.jojo("rm -rf $dir").submit()
            }
        }
        ...
        set(remove) {
            if (remove) {
                ...
                if (Const.Version.atLeast_21_2())
                    ...
                    Shell.jojo("rm -rf $PERSIST/$id").submit()
            } else {
                !removeFile.delete()
                if (Const.Version.atLeast_21_2())
                    Shell.jojo("copy_sepolicy_rules").submit()
                else
                    Shell.jojo("cp -af $ruleFile $PERSIST/$id").submit()
            }
        }

    init {
        runCatching {
            parseProps(Shell.jojo("dos2unix < $path/module.prop").exec().out)
        }
